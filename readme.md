...
> Автоматическое отслеживание позиции игрока на картах **Escape from Tarkov** без сворачивания игры и потери фокуса.
...
```
TarkovMapTracker/
├── TarkovMapTracker.cs              # Исходник C# трей-сервера (.NET Framework, WinForms)
├── TarkovMapTracker.exe             # Готовый билд сервера для Windows
├── TarkovMapTrackerAuto.user.js     # Юзерскрипт для tarkov-market.ru/maps (Tampermonkey)
├── LICENSE                          # GNU GPL v3
└── public/
    └── ico/
        ├── pngegg.png               # Иконка приложения (встраивается в .exe как ресурс)
        └── pngegg.ico               # Иконка для других нужд
```

### Технологии

- **C# / .NET Framework** — серверная часть (`System.Windows.Forms` для трея, `System.Net.Sockets` для HTTP-сервера, `System.IO.FileSystemWatcher` для мониторинга папки).
- **JavaScript (Userscript API)** — клиентская часть, `GM_xmlhttpRequest` для обхода same-origin при обращении к `127.0.0.1`.
- **SVG** — отрисовка стрелки направления поверх стандартного маркера сайта.
...
- Поддержка других картографических сайтов (tarkov.dev, mapgenie.io).
- Конфигурационный файл для порта и пути к папке скриншотов.
- Логирование всех полученных координат в CSV для анализа маршрутов.
- Веб-интерфейс для просмотра истории перемещений.
- Сборка под .NET 8 для кроссплатформенности (Linux/macOS).
